 ...
 *
 * Disassembly of dsdt.dat, Sat Oct  9 15:47:08 2021
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x00019C48 (105544)
 *     Revision         0x02
 *     Checksum         0x36
 *     OEM ID           "INTEL"
 *     OEM Table ID     "TGL     "
...
DefinitionBlock ("", "DSDT", 2, "INTEL", "TGL     ", 0x00000000)
{
    ...
    OperationRegion (FNVS, SystemMemory, 0x7DB92000, 0x000F)
    ...
    Scope (\)
    {
        ...
        Name (SS3, Zero)
        Name (SS4, One)
        OperationRegion (GNVS, SystemMemory, 0x7DB94000, 0x0AB5)
    ...
    Scope (_SB.PC00.RP06.PXSX)
    {
        ...
        {
            OperationRegion (PXCS, SystemMemory, PC2M (Arg0), 0x20)
            Field (PXCS, AnyAcc, NoLock, Preserve)
            {
                Offset (0x18), 
                PBUS,   8, 
        ...
        Device (PEPD)
        {
            ...
            Method (_DSM, 4, Serialized)  // _DSM: Device-Specific Method
            {
                If ((Arg0 == ToUUID ("c4eb40a0-6cd2-11e2-bcfd-0800200c9a66") /* Unknown UUID */))
                {
                    ...
                    If ((Arg2 == One))
                    {
                        If ((^^PC00.RP09.VDID != 0xFFFFFFFF))
                        {
                            Local0 = PFRP (0x08, Zero)
                            DerefOf (DEVY [Local0]) [Zero] = "\\_SB.PC00.RP09"
                            DerefOf (DEVY [Local0]) [0x02] = Package (0x02)
                                {
                                    Zero, 
                                    Package (0x02)
                                    {
    ...
    Scope (_SB.PC00.THC0)
    {
        ...
        Method (_CRS, 0, NotSerialized)  // _CRS: Current Resource Settings
        {
            ...
            Return (RBUF) /* \_SB_.PC00.THC0._CRS.RBUF */
        }

        PowerResource (PRTP, 0x00, 0x0000)
        {
            Method (_STA, 0, NotSerialized)  // _STA: Status
            {
                ADBG ("SB.PC00.THC0.PRTP: _STA ")
                If ((GGOV (0x0908000E) == One))
                {
                    Return (One)
                }
                Else
            ...
            Method (_ON, 0, NotSerialized)  // _ON_: Power On
            {
                ADBG ("SB.PC00.THC0.PRTP: _ON ")
                SGOV (0x0908000E, One)
            }

            Method (_OFF, 0, NotSerialized)  // _OFF: Power Off
            {
                ADBG ("SB.PC00.THC0.PRTP: _OFF ")
            }
        }
