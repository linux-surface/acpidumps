 ...
 */
DefinitionBlock ("", "SSDT", 2, "OEMHT ", "SsdtSurf", 0x00001000)
{
    ...
    OperationRegion (MNVS, SystemMemory, 0x55F49000, 0x0043)
    ...
    Scope (\_SB)
    {
        Device (SSH)
        {
            ...
            CreateDWordField (SBUF, \_SB.SSH._Y51._SPE, BAUD)  // _SPE: Speed
            ...
            Method (_DSM, 4, NotSerialized)  // _DSM: Device-Specific Method
            {
                If ((Arg0 == ToUUID ("d5e383e1-d892-4a76-89fc-f6aaae7ed5b5") /* Unknown UUID */))
                {
                    ...
                    If ((Arg1 == Zero))
                    {
                        ...
                        If ((Arg2 == 0x0B))
                        {
                            UDB0 ("SSH._DSM: Platform supports High Resolution timers for UART reads\n")
                            Return (One)
                        }
                    }
                }
    ...
    Scope (\_SB.PC00.I2C0)
    {
        Device (NFC1)
        {
            ...
            Method (_CRS, 0, NotSerialized)  // _CRS: Current Resource Settings
            {
                Return (NFCR) /* \_SB_.PC00.I2C0.NFC1.NFCR */
            }

            Method (_STA, 0, NotSerialized)  // _STA: Status
            {
                If ((\_SB.GGOV (0x0014088E) == One))
                {
                    Return (0x0F)
                }
                Else
